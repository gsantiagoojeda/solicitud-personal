...
export default async function editarSolicitud(e) {
  if (!e.target.matches("#btn-editar-solicitud")) return;
  ...
  try {
    let res = await fetch(`${SOLICITUDES}editarSolicitud.php`, options);
    ...
    if (!json.err) {
      ...
      location.replace(`${DOMAIN}ver-solicitudes.html`);
